...
def pi(request):
    ...
    ap=psyapn.objects.filter(status=1).count()
    d=psyapn.objects.filter(status=2).count()
    p=psyapn.objects.filter(status=0).count()
    cont={
        'x':ps,'d':d,'p':p,'ap':ap
    }
    return render(request,'psyindx.html',cont)

def ptanc(request):
    return render(request,'ptanc.html')
...
def pntlog(request):
    return render(request,'pntlogpg.html')
...
def apnreq(request):
    p=request.session.get('p_id')
    areq=pregdb.objects.filter(id=p)
