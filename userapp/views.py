...
def contdata(request):
    if request.method=='POST':
        ...
        sub=request.POST['cs']
        msg=request.POST['cm']
        data=contdb(uid=regdb.objects.get(id=usrid),cname=name,cmail=mail,csub=sub,cmsg=msg)
        data.save()
...
def repdata(request, id):
    if request.method == 'POST':
        try:
            rep = request.POST['rp']
            splitted_sentence = rep.split()
            words_list = [item[0] for item in bldb.objects.all().values_list('bwrd')]
            # Check if any bully words are present
            if any(word in words_list for word in splitted_sentence):
                return redirect('ureperr')  # Redirect if bully word found
            else:
                data = reply(did=topic.objects.get(id=id), rep=rep)  # Save if no bully words found
                data.save()
        except KeyError:
            # Handle case where 'rp' key is not present in POST data
            pass
        except topic.DoesNotExist:
            # Handle case where topic with given id does not exist
            pass
    return redirect('ureps', id=id)
