...
def ati(request):
    ...
    d=atapn.objects.filter(status=2).count()
    p=atapn.objects.filter(status=0).count()
    ap=atapn.objects.filter(status=1).count()
    cont={
        'x':at,'d':d,'p':p,'ap':ap
    }
...
def ntlog(request):
    return render(request,'ntlogpg.html')

def atanc(request):
    return render(request,'atanc.html')
...
def ataprs(request):
    a=request.session.get('a_id')
    ata=atregdb.objects.filter(id=a)
    cont={
        'a':ata
    }
